...
class SdsClient(object):
    ...
    def createOrUpdateStream(self, namespace_id, stream):
        ...
        req_url = self.__url + self.__streamsPath.format(tenant_id=self.__tenant, namespace_id=namespace_id, stream_id=stream.Id)
        response = requests.put(
            # self.__url + self.__streamsPath.format(tenant_id=self.__tenant, namespace_id=namespace_id, stream_id=stream.Id),
            req_url,
            data=stream.toJson(),
    ...
    def insertValue(self, namespace_id, stream_id, value):
        ...
        path = self.__insertValuePath.format(tenant_id=self.__tenant, namespace_id=namespace_id,
                                             stream_id=stream_id)
        req_url = self.__url + path
        hdrs = self.__sdsHeaders()
        response = requests.post(req_url, data=payload, headers=hdrs)
        if response.status_code < 200 or response.status_code >= 300:
            response.close()
